...
const byte divePin = 8, humDirtPin = A1, photoResPin;
...
void loop() {
  ...
  time_f += time_h;
  if((time_f >= 9.0000 && time_f <= 22.0000) || true){
...
void dht_print(){
  ...
  switch(dht.getState()) {
    ...
    case DHT_OK:
      ...
      break;
    // РѕС€РёР±РєР° РєРѕРЅС‚СЂРѕР»СЊРЅРѕР№ СЃСѓРјРјС‹
    case DHT_ERROR_CHECKSUM:
      Serial.println("Checksum error");
      data_error = true;
      if(displayOutput_change) displayDigits(DISPLAY_PIN, QD_E, QD_r, QD_r, QD_NONE);
      break;
    // РїСЂРµРІС‹С€РµРЅРёРµ РІСЂРµРјРµРЅРё РѕР¶РёРґР°РЅРёСЏ
    case DHT_ERROR_TIMEOUT:
      Serial.println("Time out error");
      data_error = true;
      if(displayOutput_change) displayDigits(DISPLAY_PIN, QD_E, QD_r, QD_r, QD_NONE);
      break;
    // РґР°РЅРЅС‹С… РЅРµС‚, РґР°С‚С‡РёРє РЅРµ СЂРµР°РіРёСЂСѓРµС‚ РёР»Рё РѕС‚СЃСѓС‚СЃС‚РІСѓРµС‚
